...
- [x] T1: Bump `qa-commons-api` to v0.3.0, add `qa-commons-db` v0.3.0 (test
...
- [x] T2: Relocate Allure output — surefire gets
...
- [x] T3: Start the service via `docker-compose up`, fetch `/q/openapi`, and
...
- [x] T4: Add the DB oracle — `NotificationsOracle`, `NotificationRow`,
...
- [x] T5: Add `NotificationsEndpoint` (send) — files:
...
- [x] T6: `SendNotificationLiveTest` — positive (202, response shape) and
...
- [x] T7: `SendNotificationOracleLiveTest` — send, then assert the row
...
- [x] T8: `DuplicateSendLiveTest` — two identical payloads → two 202s,
...
- [x] T9: `InvalidChannelFindingLiveTest` + findings doc (entry 1 of 3) —
...
- [x] T10: `FailedNotificationsEndpoint` + `FailedNotificationsPageResponse`
...
- [x] T11: `MetricsEndpoint` + `MetricsTodayContractLiveTest` — envelope
...
- [x] T12: README two-tier testing paragraph — files: `README.md` — done
...
- [x] T13: Close-out verification — run `mvn test` with the service down
